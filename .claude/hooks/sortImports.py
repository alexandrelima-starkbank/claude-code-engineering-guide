...
def extractImportBlock(lines):
    startIndex = None
    endIndex = None  # pragma: no mutate
...
def sortImports(filePath, localPackages=None):
    ...
    if not sortedLines:  # pragma: no mutate
        path.write_text(''.join(lines[:startIndex] + lines[endIndex:]))  # pragma: no mutate
...
def sortLocalGroup(lines):
    ...
    order = []
    for line in lines:
        topLevel = getTopLevelModule(line) or ''  # pragma: no mutate
    ...
    sortedNs = sorted(order, key=lambda ns: max(len(l) for l in namespaceGroups[ns]), reverse=True)
    result = []
    for ns in sortedNs:
